<template>
	<div class="uploadWrap">
         <a href="javascript:;" class="fileBtn">
            <input v-show="disUpload" class="fileInput" :disabled="disabled"  @change="posterImg($event)" :id="dataId" type="file" :data-id="dataId" name="signFile" />
            <span v-show="!IsCover">上传封面</span>
        </a>
        <div v-if="fileShow" style="display:block;cursor:pointer;">{{fileValue.name}}</div>
...
import { strToObj } from '../../util/string.js';
	export default{
        props:['dataId','imgSrc','disabled','IsCover',"disUpload"],
		data () {
			return {
                index:0,
                fileShow:false,
