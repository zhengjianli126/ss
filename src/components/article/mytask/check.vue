...
<div class="checkWarp">
    <el-container>
        <el-main>
            <el-form  status-icon ref="info" label-width="100px" class="demo-ruleForm">
                <el-form-item label="文章标题:" prop="desc">
                    <div style="width:500px;float:left; margin-right:50px ">
                        <el-input type="text" v-model="info.articleTitle" autocomplete="off"></el-input>
                    </div>

                    <el-button style="float:left" @click="stop">放弃审稿</el-button>
                </el-form-item>
                ...
                <el-form-item label="标签:" prop="articleTags">
                    <div class="checktags">
                        ...
                        <el-button class="button-new-tag" size="small" @click="gettags">+ New Tag</el-button>
                    </div>
                </el-form-item>
                <el-form-item label="文章内容:" prop="articleContent">
                    <wangEditor :editorContent="info.articleContent"></wangEditor>
                    <!-- <vue-ueditor-wrap v-model="info.articleContent"></vue-ueditor-wrap> -->
                </el-form-item>
                <el-form-item label="封面图:" prop="articleImgPath">
                    <el-upload class="avatar-uploader" action="http://39.106.49.2:8078/upload/upload_img?suffix=.jpg" :show-file-list="false" :on-success="handleAvatarSuccess" :before-upload="beforeAvatarUpload">
                        <img style='max-width:400px;max-height:400px;' v-if="info.articleImgPath" :src="info.articleImgPath" class="avatar">
                        <i v-else class="el-icon-plus avatar-uploader-icon"></i>
                    </el-upload>
                </el-form-item>
                <el-form-item label="视频上传:" prop="videoPath">
                    <!-- action必选参数, 上传的地址 -->
                    <el-upload class="avatar-uploader" action="http://39.106.49.2:8078/upload/upload_img?suffix=.mp4" :show-file-list="false" :on-success="vhandleAvatarSuccess" :before-upload="vbeforeAvatarUpload">
                        <video v-if="info.videoPath" controls="controls" :src="info.videoPath&&info.videoPath!==''" class="avatar"></video>
                        <i v-else class="el-icon-plus avatar-uploader-icon"></i>
                    </el-upload>
...
export default {
    ...
    methods: {
        vhandleAvatarSuccess(res, file) {
            ...
            console.log(this.info.videoPath)
        },
        ...
        handleAvatarSuccess(res, file) {
            if (res.respCode == '0') {
                ...
                this.info.articleImgPath = res.respMsg
            } else {
                  this.$message({
                    showClose: true,
                    message: '上传失败',
