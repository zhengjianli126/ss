<template>
 <div class="taskListWarp" > 
      <el-table
        ...
        width="165">
        <template slot-scope="scope">
            <el-button v-if="scope.row.status==='审核中'||scope.row.status==='被退稿'" @click="update(scope.row.articleId,scope.row.nowlevel)" type="text" size="small">审核</el-button>
...
import { getArticleInfo,getArticleOperate,revokeCheckBackArticle } from '../../../service/getData.js'//引入接口
  export default {
    props:['isLoading'],
    data() {
      return {
   ...
   methods:{
     ...
     update(a,b ){
            ...
            params.articleId=a
            params.checkStatus=b
             this.$router.push({name:"mytasktasktotal",  query:{...params}})
     },
